...
class SgldBayesianRegression:
    ...
    def __init__(
        ...
        Args:
            ...
            a_beta: Shape parameter for the inverse gamma prior of sigma^2_beta.
            b_beta: Rate parameter for the inverse gamma prior of sigma^2_beta.
    ...
    def _log_prob_of_prior(self, beta, sigma_beta_squared):
        """
        Compute the probability of beta under a Gaussian prior N(0, sigma_beta^2 I).
        """
        # print(f'beta.shap={beta.shape}')
        if len(beta.shape) == 1:
            beta = beta.unsqueeze(0)  # torch.Size([2]) --> torch.Size([1, 2])
        # print(f'beta.shap={beta.shape}')
    ...
    def _sample_sigma_beta_squared(self):
        ...
        with torch.no_grad():
            ...
            beta = torch.cat([p.flatten() for p in self.model.parameters()]).to(self.device)  # Flatten all parameters into a vector
            ...
            # print(f"Sampled σβ^2: {sigma_beta_squared.item()}")
